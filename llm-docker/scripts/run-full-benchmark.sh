...
if [ "$SKIP_PULL" = false ]; then
    ...
    INSTALLED_COUNT=$(docker exec "$OLLAMA_CONTAINER" ollama list 2>/dev/null | grep -E "llama3|mistral|qwen|deepseek|gemma|phi|codellama|nemotron|ministral|qwq|granite|dolphin|codestral|falcon|glm4|exaone|aya|olmo|hermes|marco|smollm|gpt-oss|devstral" | wc -l)
    TOTAL_MODELS=59
